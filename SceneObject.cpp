...
glm::vec3 SceneObject::lighting(glm::vec3 lightPos, glm::vec3 viewVec, glm::vec3 hit, bool multipleLights)
{
	const float intensity = 0.4f;
	float ambientTerm = ambient;
	if (multipleLights) ambientTerm = 0.0;
	float diffuseTerm = 0;
	...
	glm::vec3 colorSum = (ambientTerm * color_) + ((lDotn * color_) + (specularTerm * glm::vec3(1)))* intensity;
